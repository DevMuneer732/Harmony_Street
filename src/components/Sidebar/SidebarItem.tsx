...
const SidebarItem: React.FC<any> = ({ item }: any) => {
  ...
  const isChildActive = item.children?.some(
    (child: any) => pathname === child.route || pathname.startsWith(child.route + '/')
  );
      ...
      <li>
        <Link
          ...
          className={`${isActive || isChildActive
              ? "bg-primary/[.07] text-primary dark:bg-primary dark:text-white"
        ...
        >
          ...
          {item.children && (
            <svg
              className={`absolute right-3.5 top-1/2 -translate-y-1/2 fill-current transition-transform ${isOpen ? "rotate-180" : ""
                }`}
              width="22"
              height="22"
              viewBox="0 0 22 22"
